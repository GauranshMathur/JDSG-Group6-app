...
| F-6.3 | Search behaves identically on SQLite and PostgreSQL | Met — but **only since [#97](https://github.com/GauranshMathur/JDSG-Group6-app/pull/97)**. It was recorded as met while `Post.search` folded case on neither side, so it worked by accident of SQLite's LIKE and would have broken on the switch (finding 2 of the [2026-08-18 review](docs/architecture-reviews/2026-08-18.md)). Both scopes now fold case in SQL and name an ESCAPE character. Proven twice over: by specs that set `PRAGMA case_sensitive_like = ON` to make SQLite compare as PostgreSQL does, and **since [#105](https://github.com/GauranshMathur/JDSG-Group6-app/pull/105) by running the whole suite against a real PostgreSQL 16 in CI** — 393 examples, green on both adapters. The first PostgreSQL run failed 310 of 393, and every one of those was the suite rather than the app: a SQLite-only `PRAGMA` in a spec hook, which PostgreSQL rejects as a syntax error, and a rejected statement there aborts the whole transaction |
...
| N-5.3 | Sign-in attempts are rate limited | Met — `rate_limit to: 10, within: 3.minutes` on sign-in, password reset and registration, and **since [#103](https://github.com/GauranshMathur/JDSG-Group6-app/pull/103) there are specs that reach all three**. Until then the test environment used `:null_store`, so the counter was discarded and zero specs could exercise the limit — the protection was real in production and untested everywhere. It still counts through `Rails.cache`, so the limit is per-process and resets on restart; a shared store is needed before it means anything under more than one instance |
...
| ID | Requirement | Status |
| --- | --- | --- |
| N-6.1 | Rendering the feed issues a constant number of queries, regardless of how many posts are on the page | Met — 2 signed out (posts + reposters; a page showing no reposts drops to 1), 5 signed in (adding the session and the batched like and repost lookups), flat from 1 post to a full page and re-measured warm against the 10k seed with the ranked window in place. Profile pages hold the same property at 2 signed out and 5 signed in, the extra query being the username lookup |
